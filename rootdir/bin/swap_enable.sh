...
function configure_para_casio()
{
	...
	case "$product_id" in
		"24606"|"24678"|"24679"|"24680"|"24681")
			if [ $MemTotal -le 6291456 ]; then
				echo 55 > /proc/sys/vm/watermark_scale_factor
			else
				echo 75 > /proc/sys/vm/watermark_scale_factor
			fi
	esac
	...
	case "$product_id" in
		"24921"|"24922"|"24971"|"24972")
			if [ $MemTotal -le 8388608 ]; then
				echo 67 > /proc/sys/vm/watermark_scale_factor
			else
...
function configure_memory_parameters() {
	...
	# must write any original watermark knob to trigger a watermark update
	echo 20480 > /sys/module/oplus_bsp_zram_opt/parameters/oplus_extra_free_kbytes
